#ifndef __CLOUD_STORAGE_HTTPUPLOADER_HPP__
#define __CLOUD_STORAGE_HTTPUPLOADER_HPP__
...
public:
  ...
  /**
   * @brief 负责根据请求的路径和方法查找匹配的路由，并调用相应的处理函数
   * @param conn 与客户端的TCP连接
   * @param req 客户端发送的HTTP请求
   * @param resp 用于构造和发送HTTP响应给客户端
   * @return 返回true表示同步处理完成,false表示异步处理
   */
  bool onRequest(const TcpConnectionPtr &conn, HttpRequest &req,
                 HttpResponse *resp) {
    std::string path = req.path();    // 获取客户端HTTP请求的路径
...
private:
  ...
  /**
   * @brief 处理HTTP POST请求中的文件上传操作, 需要验证用户的会话session
   * conn 解析请求中的文件信息,创建或使用已有的上传上下文,将文件内容分块写入文件
   * 并在文件上传完成之后进行相应的处理
   * @param conn 客户端的TCP连接指针
   * @param req 客户端的HTTP请求
   * @note 传大文件时,客户端将文件分割成多数据块进行传入,服务器端也要相应的分块处理, 可以通过解析multipart/form-data的边界
   * 来实现对文件的分块处理. 具体的, 在接收到上传请求的时,该函数会从请求的Content-Type头部中提取出边界字符串,根据边界字符串,函数
   * 判断数据块的开始和结束位置,将文件数据分块写入到文件中
   */
...
private:
  ...
  //* @brief 初始化各种路由,后续客户端要请求的数据都要来此路由进行查找, 会根据不同的路由来执行不同的handler
  void initRoutes() {
    // 不需要会话验证的路由
  ...
  // 登出处理
  // FIXME TODO: 用户如果直接退出浏览器也需要进行会话session的删除
